...
const peopleBornOnFifteen = inventors.filter(
  (person) => person.year >= 1500 && person.year < 1600
);
console.table(peopleBornOnFifteen);
// Array.prototype.map()
...
// const deNames = links.map(link => link.textContent).filter(name => name.includes('de'))
// 7. sort Exercise
...
const instances = data.reduce((obj, curr) => {
